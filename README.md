...
| **Умный календарь с разворачиванием** | В календаре показывается до 3 записей, клик `+N more` разворачивает день, показывая все запланированные видео. |
| **Grace-период для просроченных** | 2-минутный грейс: видео, ставшие due при работающем диспетчере, публикуются; давно просроченные (сервер был выключен) помечаются `overdue` без авто-публикации. |
| **Publish Now для overdue** | В календаре у просроченных/ошибочных записей зелёная кнопка "Publish Now" — публикация в 1 клик. |
| **Фикс шифрования токенов YouTube** | Токены YouTube хранятся в localStorage зашифрованными. `BatchScheduleModal` и `ScheduleCalendar` теперь расшифровывают их перед отправкой на бэкенд. |
| **Умная длительность клипов** | AI-детекция виральных моментов адаптирует длину клипа под видео: короткие (<2мин) → 30-90с, средние → 50-120с, длинные → до 5мин. |
